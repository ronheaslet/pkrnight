...
export default function TrophiesTab() {
  ...
  const fetchSeasonData = async () => {
    ...
    setHistoricalWinners(yearlyStats)
    setLoading(false)
  }
  ...
  const calculateYearlyStats = (results, members) => {
    ...
    Object.entries(byYear).forEach(([year, yearResults]) => {
      ...
      if (pointsWinner) {
        winners.push({
          year: parseInt(year), category: 'points', userId: pointsWinnerId,
      ...
      if (bountyWinner) {
        winners.push({
          year: parseInt(year), category: 'bounty', userId: bountyWinnerId,
  ...
  return (
    <div className="px-4 py-4">
          ...
          {leaders.points && (
            <div className="text-center">
              ...
              <div className="flex items-center gap-2 justify-center">
                <div className="w-6 h-6 rounded-full bg-gold text-felt-dark flex items-center justify-center text-[10px] font-bold">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {isAdmin && <button onClick={() => setShowAddWinnerModal(true)} className="btn btn-secondary text-xs py-1 px-3">+ Add Winner</button>}
      </div>

      {/* Trophy Categories */}
      {['points', 'bounty'].map(category => {
        const info = getCategoryInfo(category)
        const categoryWinners = historicalWinners.filter(w => w.category === category)
          ...
          <div key={category} className="mb-6">
            ...
            <div className="space-y-2">
              {categoryWinners.map((winner, idx) => (
                <div key={`${winner.year}-${winner.category}`} className={`flex items-center gap-3 p-3 rounded-xl border ${getColorClasses(info.color, winner.isCurrent)}`}>
                  <div className={`font-display text-lg ${winner.isCurrent ? 'text-gold' : 'text-white/40'}`} style={{ width: 50 }}>{winner.year}</div>
...
function AddWinnerModal({ leagueId, onClose, onSave }) {
  ...
  const handleSave = async () => {
    // TODO: Save to trophy_winners table when implemented
    console.log('Save winner:', { selectedMember, category, year, stats })
    onSave()
  }
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <select value={selectedMember} onChange={(e) => setSelectedMember(e.target.value)} className="input">
              ...
              {members.map(m => (
                <option key={m.user_id} value={m.user_id}>{m.users?.display_name || m.users?.full_name}</option>
              ))}
            </select>
          </div>
          <div>
            <label className="block text-sm text-white/60 mb-1">Stats (e.g., "142 points")</label>
            <input type="text" value={stats} onChange={(e) => setStats(e.target.value)} className="input" placeholder="142 points" />
          </div>
          <button onClick={handleSave} className="w-full btn btn-primary py-3">Save Champion</button>
        </div>
      </div>
    </div>
  )
